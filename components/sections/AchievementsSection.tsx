  ...
  return (
    <div className="h-full overflow-y-auto px-14 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="mb-8">
        <span className="text-violet-400 text-xs font-bold uppercase tracking-[4px]">04.</span>
        ...
        {achievements.map((a, i) => (
          ...
          >
            ...
            <div className="absolute -left-8 top-5 w-4 h-4 rounded-full flex items-center justify-center text-[10px]"
              style={{ background: "rgba(139,92,246,0.2)", border: "1.5px solid rgba(139,92,246,0.5)", boxShadow: "0 0 8px rgba(139,92,246,0.3)" }}>
            </div>

            <Card className="glass rounded-2xl p-5 transition-all duration-300 hover:-translate-y-1 hover:border-violet-500/30 border-0 bg-transparent shadow-none">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-3 mb-1">
                  <h3 className="text-white font-bold text-base flex items-center gap-2">
                    {a.title}
                    {a.link && (
                      <a href={a.link} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-cyan-300 transition-colors">
                        <ExternalLink size={14} />
                      </a>
                    )}
                  </h3>
                  <Badge variant="outline" className="px-2.5 py-0.5 rounded-full text-xs font-bold flex-shrink-0 border-violet-500/25 text-violet-400"
