  ...
  return (
    <div className="h-full overflow-y-auto px-14 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="mb-8">
        <span className="text-red-400 text-xs font-bold uppercase tracking-[4px]">05.</span>
      ...
      <div className="grid grid-cols-3 gap-5">
        {projects.map((proj, i) => (
          <motion.div key={proj.title}
            ...
            style={{ "--hover-shadow": "0 20px 50px rgba(220,38,38,0.12)" } as React.CSSProperties}
          >
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="mt-2">
                  ...
                  <p className="text-red-400 text-xs font-semibold mt-0.5">{proj.subtitle}</p>
                </div>
              </div>
              <div className="flex gap-2 mt-1">
                <a href={proj.github} target="_blank" rel="noopener noreferrer"
                  className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:text-red-300 transition-colors"
                  style={{ background: "rgba(220,38,38,0.08)" }}>
                  <FaGithub size={15} />
                </a>
                <a href={proj.live} target="_blank" rel="noopener noreferrer"
                  className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:text-blue-300 transition-colors"
            ...
            {/* Tech Tags */}
            <div className="flex flex-wrap gap-1.5 pt-3 border-t border-red-500/10 mt-auto">
              {proj.tech.map((t) => (
                <Badge key={t} variant="outline"
                  className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold text-red-300 border-red-500/20"
                  style={{ background: "rgba(220,38,38,0.08)" }}>
                  {t}
                </Badge>
              ))}
